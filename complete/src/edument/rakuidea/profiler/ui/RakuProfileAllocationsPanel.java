...
import edument.rakuidea.profiler.model.RakuProfileModelWithRatio;
...
import static edument.rakuidea.profiler.ui.RakuProfileGCPanel.parseAndCompare;

public class RakuProfileAllocationsPanel extends JPanel {
    ...
    public RakuProfileAllocationsPanel(RakuProfileData data) {
    ...
    private void setup() {
        ...
        allocationsTable.setModel(new RakuProfileAllocationsTableModel(allocsData));
    ...
    private void updateAllocationData() {
        ...
        if (typeRow >= 0 && allocationsTable.getModel().getRowCount() > 0) {
            RakuProfileAllocationsTableModel model = (RakuProfileAllocationsTableModel)allocationsTable.getModel();
            int typeId = model.getAllocationId(typeRow);
            List<AllocatedTypeDetails> list = myProfileData.getAllocatedTypeData(typeId);
            typeDetailsTable.setModel(new RakuProfileTypeDetailsTableModel(list));
    ...
    private static class RakuProfileAllocationsTableModel implements TableModel, RakuProfileModelWithRatio {
        final List<AllocationData> allocsData;

        RakuProfileAllocationsTableModel(List<AllocationData> allocsData) {
    ...
    private static class RakuProfileTypeDetailsTableModel implements TableModel, RakuProfileModelWithRatio {
        final List<AllocatedTypeDetails> typeDetailsList;

        RakuProfileTypeDetailsTableModel(List<AllocatedTypeDetails> list) {
