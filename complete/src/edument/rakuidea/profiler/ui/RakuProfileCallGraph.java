package edument.rakuidea.profiler.ui;
...
import edument.rakuidea.profiler.model.RakuProfileCall;
...
public class RakuProfileCallGraph extends JPanel {
    ...
    private final RakuProfileCallGraphPanel myGraphPanel;
    private RakuProfileCall myRoot;
    ...
    public RakuProfileCallGraph(Project project,
                                RakuProfileData profileData,
                                RakuProfileCallGraphPanel panel) {
        myProject = project;
        ...
        MouseAdapter clickAdapterHandler = new MouseAdapter() {
            ...
            private void showTooltip(Point point, CallItem item) {
                ...
                currentPopup = JBPopupFactory.getInstance()
                        ...
                        .setFocusOwners(new Component[] { RakuProfileCallGraph.this })
                        .createPopup();
                currentPopup.show(new RelativePoint(RakuProfileCallGraph.this, point));
            }
    ...
    private int drawParentBreadcrumbs(Graphics2D g, int height) {
        ...
        if (myRoot.getParent() != null) {
            Deque<RakuProfileCall> parents = new ArrayDeque<>();
            RakuProfileCall nextParent = myRoot.getParent();
            while (nextParent != null) {
                parents.addFirst(nextParent);
                nextParent = nextParent.getParent();
            }
            for (RakuProfileCall call : parents) {
    ...
    private void updateAxis() {
        if (myRoot != null && myGraphPanel != null) {
            String overallTimeString = String.format("Time: %s μs", RakuUtils.formatDelimiters(myRoot.getInclusiveTime(), ",", 3));
    ...
    private boolean drawCall(Graphics2D g, RakuProfileCall root, int startX, int callRectWidth, int height) {
    ...
    private boolean isRootExternal(RakuProfileCall root) {
    ...
    private void drawChildren(Graphics2D g, RakuProfileCall call, Color color, int callStartX, int maxItemWidth, int lineHeight) {
        ...
        for (RakuProfileCall childCall : call.getCallees()) {
    ...
    public static class CallItem {
        ...
        public final RakuProfileCall myCall;

        public CallItem(Rectangle area, RakuProfileCall call) {
            myArea = area;
            myCall = call;
        }
