...
public abstract class Cursor<TCursor extends Cursor> {
    ...
    * first need for it. Contains groups of 4 integers, which are:
    ...
    public void endSymbol(String type) {
        ...
        Map<String, RakuSymbolKind> st = stack.symbols.get(stack.symbols.size() - 1);
        if (type.equals("term") || type.equals("type"))
            st.put(symbol, RakuSymbolKind.TypeOrConstant);
        else if (type.equals("routine"))
            st.put(symbol, RakuSymbolKind.Routine);
    }
    ...
    public boolean isName() {
        ...
        String symbol = stack.target.subSequence(stack.symbolStart, pos).toString();
        List<Map<String, RakuSymbolKind>> symbolTables = stack.symbols;
        for (int i = symbolTables.size() - 1; i >= 0; i--) {
            Map<String, RakuSymbolKind> st = symbolTables.get(i);
            RakuSymbolKind foundKind = st.get(symbol);
            if (foundKind != null)
                return foundKind == RakuSymbolKind.TypeOrConstant;
        }
