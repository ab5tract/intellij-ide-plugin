...
public class RakuWordsScanner extends VersionedWordsScanner {
    ...
    public RakuWordsScanner() {
        ...
        myIdentifierTokenSet = TokenSet.create(
          edument.rakuidea.parsing.RakuTokenTypes.NAME, edument.rakuidea.parsing.RakuTokenTypes.SUB_CALL_NAME,
          edument.rakuidea.parsing.RakuTokenTypes.METHOD_CALL_NAME, edument.rakuidea.parsing.RakuTokenTypes.VARIABLE,
          edument.rakuidea.parsing.RakuTokenTypes.ROUTINE_NAME, edument.rakuidea.parsing.RakuTokenTypes.PACKAGE_NAME
        );
        myCommentTokenSet = TokenSet.create(edument.rakuidea.parsing.RakuTokenTypes.COMMENT, edument.rakuidea.parsing.RakuTokenTypes.POD_TEXT, edument.rakuidea.parsing.RakuTokenTypes.POD_FINISH_TEXT);
        myLiteralTokenSet = TokenSet.create(
          edument.rakuidea.parsing.RakuTokenTypes.INTEGER_LITERAL, edument.rakuidea.parsing.RakuTokenTypes.COMPLEX_LITERAL,
          edument.rakuidea.parsing.RakuTokenTypes.NUMBER_LITERAL, edument.rakuidea.parsing.RakuTokenTypes.RAT_LITERAL);
    }
    ...
    public void processWords(@NotNull CharSequence fileText, @NotNull Processor<? super WordOccurrence> processor) {
        ...
        while ((type = myLexer.getTokenType()) != null) {
            if (myIdentifierTokenSet.contains(type)) {
                ...
                if (type == edument.rakuidea.parsing.RakuTokenTypes.VARIABLE) {
                    if (text.length() > 1) {
                ...
                } else if (type == edument.rakuidea.parsing.RakuTokenTypes.NAME) {
                    ...
                    if (!processor.process(occurrence)) return;
                } else if (type == edument.rakuidea.parsing.RakuTokenTypes.METHOD_CALL_NAME || type == edument.rakuidea.parsing.RakuTokenTypes.ROUTINE_NAME ||
                           type == edument.rakuidea.parsing.RakuTokenTypes.SUB_CALL_NAME) {
                    if (text.indexOf('-') == -1) {
                        occurrence.init(text, 0, text.length(), WordOccurrence.Kind.CODE);
                        if (!processor.process(occurrence)) return;
                    } else {
