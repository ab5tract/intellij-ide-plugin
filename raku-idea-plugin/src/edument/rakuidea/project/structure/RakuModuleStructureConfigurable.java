...
public class RakuModuleStructureConfigurable extends RakuStructureConfigurable implements Place.Navigator {
    ...
    public RakuModuleStructureConfigurable(Project project, RakuStructureConfigurableContext context) {
        super(project);
    ...
    private RakuDistNode createDistNode(Module module, ModuleGrouper moduleGrouper) {
        RakuModuleConfigurable configurable =
            new RakuModuleConfigurable(myContext.myModulesConfigurator, module, TREE_UPDATER, moduleGrouper);
    ...
    @Override
    public void init(RakuStructureConfigurableContext context) {
        super.init(context);
    }
    ...
    private void addModule(boolean anImport) {
        ...
        if (modules != null && !modules.isEmpty()) {
            ...
            edument.rakuidea.project.structure.RakuProjectStructureConfigurable.getInstance(myProject).getProjectSdksModel().syncSdks();
    ...
    private class RakuDistRemoveHandler extends RemoveConfigurableHandler<Module> {
        RakuDistRemoveHandler() {
            super(RakuModuleConfigurable.class);
        }

        @Override
        public boolean remove(List<?> modules) {
            RakuModulesConfigurator modulesConfigurator = myContext.myModulesConfigurator;
