...
import edument.rakuidea.parsing.Cursor;
import edument.rakuidea.parsing.CursorStack;
...
public class CroTemplateLexer extends LexerBase {
        ...
        while (true) {
            ...
            switch (outcome) {
                case -1: {
                    ...
                    if (stack.isEmpty()) {
                        if (c.pos != stack.target.length())
                            System.err.println("Cro Template lexer: failed to lex the whole file (passed early)");
                        stack.token = null;
                        return;
                    }
                    continue;
                ...
                case -2:
                    stack.pop();
                    if (stack.isEmpty()) {
                        System.err.println("Cro Template lexer: failed to lex the whole file (failed to match)");
                        stack.token = null;
                        return;
                    }
                    continue;
                case -3:
                    if (stack.tokenStart < lastTokenStart)
                        System.err.println("Cro Template lexer went backwards (from " + lastTokenStart +
