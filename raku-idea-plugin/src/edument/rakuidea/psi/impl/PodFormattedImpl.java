...
import edument.rakuidea.pod.PodDomFormatted;
import edument.rakuidea.pod.PodDomNode;
import edument.rakuidea.pod.PodDomText;
import edument.rakuidea.psi.PodFormatted;
...
public class PodFormattedImpl extends ASTWrapperPsiElement implements PodFormatted {
    ...
    public TextRange getFormattedTextRange() {
        PsiElement starter = findChildByType(edument.rakuidea.parsing.RakuTokenTypes.POD_FORMAT_STARTER);
        PsiElement stopper = findChildByType(edument.rakuidea.parsing.RakuTokenTypes.POD_FORMAT_STOPPER);
        ...
        if (content != null) {
            for (ASTNode child : content.getNode().getChildren(TokenSet.ANY)) {
                if (inLink) {
                    ...
                    if (elementType == edument.rakuidea.parsing.RakuTokenTypes.POD_FORMAT_SEPARATOR) {
