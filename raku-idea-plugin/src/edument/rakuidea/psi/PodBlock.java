...
public interface PodBlock extends PodElement {
    TokenSet POD_CONTENT = TokenSet.create(
      RakuElementTypes.POD_BLOCK_ABBREVIATED,
      RakuElementTypes.POD_BLOCK_DELIMITED,
      RakuElementTypes.POD_BLOCK_PARAGRAPH,
      RakuElementTypes.POD_FORMATTED,
      edument.rakuidea.parsing.RakuTokenTypes.POD_TEXT,
      edument.rakuidea.parsing.RakuTokenTypes.POD_CODE,
      edument.rakuidea.parsing.RakuTokenTypes.POD_NEWLINE);

    @Nullable
    default String getTypename() {
        ASTNode typenameNode = getNode().findChildByType(edument.rakuidea.parsing.RakuTokenTypes.POD_TYPENAME);
    ...
    default PodDomNode buildPodDom(PodDomBuildingContext context) {
        ...
        for (ASTNode child : getContent()) {
            ...
            if (child.getElementType() == RakuTokenTypes.POD_REMOVED_WHITESPACE)
                continue;
            ...
            if (child.getElementType() == edument.rakuidea.parsing.RakuTokenTypes.POD_NEWLINE) {
                ...
                if (outstandingNewlines == 1) {
                    ...
                    // newlines.
                    if (node instanceof PodDomCode && child.getElementType() == edument.rakuidea.parsing.RakuTokenTypes.POD_CODE) {
            ...
            if (node == null) {
                if (child.getElementType() == edument.rakuidea.parsing.RakuTokenTypes.POD_CODE) {
            ...
            else if (child.getElementType() == edument.rakuidea.parsing.RakuTokenTypes.POD_CODE)
