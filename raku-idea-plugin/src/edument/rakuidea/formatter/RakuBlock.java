package edument.rakuidea.formatter;
...
import static edument.rakuidea.parsing.RakuOPPElementTypes.INFIX_APPLICATION;
import static edument.rakuidea.parsing.RakuTokenTypes.*;
...
class RakuBlock extends AbstractBlock implements BlockWithParent {
    ...
    private RakuBlock(ASTNode node, Wrap wrap, Alignment align, Boolean isStatementContinuation,
                      CommonCodeStyleSettings commonSettings, RakuCodeStyleSettings customSettings,
                      List<BiFunction<RakuBlock, RakuBlock, Spacing>> rules) {
        super(node, wrap, align);
        myRules = rules;
    ...
    public Wrap getWrap() {
        ...
        if (myNode.getPsi() instanceof RakuMethodCall && myNode.getText().startsWith(".") && myCustomSettings.METHOD_CALL_WRAP)
            return Wrap.createWrap(WrapType.NORMAL, false);
        if (myNode.getTreeParent() != null && myNode.getTreeParent().getPsi() instanceof RakuInfixApplication application &&
            myNode.getElementType() != edument.rakuidea.parsing.RakuTokenTypes.NULL_TERM && myNode.getElementType() != RakuElementTypes.INFIX) {
            ...
            if (application.getOperator().equals(",") && (parent instanceof RakuSubCall || parent instanceof RakuMethodCall))
    ...
    private Pair<Function<ASTNode, Boolean>, Alignment> calculateAlignment(ASTNode node) {
        ...
        if (type == SIGNATURE && myCustomSettings.PARAMETER_ALIGNMENT) {
            ...
            if (!(node.getPsi() instanceof RakuInfixApplication infixApp))
                return null;
            ...
            if (infixApp.getOperator().equals(",")) {
                ...
                PsiElement origin = PsiTreeUtil.getParentOfType(infixApp, RakuVariableDecl.class, RakuCodeBlockCall.class, RakuArrayComposer.class, RakuStatement.class);
                // This case is handled by another option
                if (origin instanceof RakuArrayComposer)
                    return null;
                if (origin instanceof RakuCodeBlockCall)
                    return myCustomSettings.CALL_ARGUMENTS_ALIGNMENT ? Pair.create(
                    (child) -> child.getElementType() != edument.rakuidea.parsing.RakuTokenTypes.INFIX && child.getElementType() != edument.rakuidea.parsing.RakuTokenTypes.NULL_TERM,
                    Alignment.createAlignment()) : null;
                else
                    return myCustomSettings.ARRAY_ELEMENTS_ALIGNMENT ? Pair.create(
                    (child) -> child.getElementType() != edument.rakuidea.parsing.RakuTokenTypes.INFIX && child.getElementType() != RakuTokenTypes.NULL_TERM,
                    Alignment.createAlignment()) : null;
            }
    ...
    public Indent getIndent() {
        ...
        if (myNode.getTreeParent().getPsi() instanceof RakuBlockoid && myNode.getTreeNext() != null && myNode.getTreePrev() != null)
        ...
        if (isStatementContinuation != null && isStatementContinuation) {
            if (myNode.getElementType() == PARENTHESES_CLOSE && myNode.getTreeParent().getPsi() instanceof RakuSignature) {
                return Indent.getSpaceIndent(1, true);
            }
            else if (myNode.getElementType() == PARENTHESES_CLOSE && myNode.getTreeParent().getPsi() instanceof RakuSubCall subCall) {
                if (subCall.getCallArguments().length != 0) {
                    PsiElement infix = subCall.getCallArguments()[0].getParent();
                    return Indent.getSpaceIndent(infix instanceof RakuInfixApplication ? infix.getStartOffsetInParent() : 0, true);
                }
            }
    ...
    private static boolean nodeInStatementContinuation(ASTNode startNode) {
        ...
        if (startPsi instanceof RakuTrait)
            return true;
            ...
            if (hopefullyStatement instanceof RakuStatement) {
                ...
                if (statementHolder instanceof RakuStatementList) {
                    ...
                    if (hopefullyPairish instanceof RakuFatArrow || hopefullyPairish instanceof RakuColonPair) {
                        ...
                        if (hopefullyBlockoid instanceof RakuBlockoid &&
                            hopefullyBlockoid.getParent() instanceof RakuBlockOrHash)
                            return true;
                    }
                }
        ...
        if (elementType == REGEX_GROUP || elementType == ARRAY_COMPOSER) {
            ...
            while (block != null) {
                ...
                if (subblocks.size() != 0) {
                    ...
                    if (block instanceof RakuBlock && ((RakuBlock)block).getNode().getElementType() == UNTERMINATED_STATEMENT)
