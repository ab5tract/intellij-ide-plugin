...
import edument.rakuidea.annotation.fix.AddUnitDeclaratorQuickFix;
import edument.rakuidea.annotation.fix.RemoveUnitDeclaratorQuickFix;
import edument.rakuidea.parsing.RakuTokenTypes;
import edument.rakuidea.psi.*;
import org.jetbrains.annotations.NotNull;

import static edument.rakuidea.parsing.RakuTokenTypes.UNV_WHITE_SPACE;
...
public class MissingUnitKeywordAnnotator implements Annotator {
    ...
    public void annotate(@NotNull PsiElement element, @NotNull AnnotationHolder holder) {
        if (element instanceof RakuScopedDecl) {
            ...
            if (node.getElementType() == edument.rakuidea.parsing.RakuTokenTypes.STATEMENT_TERMINATOR) {
                PsiElement packageDeclarator = element.getFirstChild();
                String declaratorType = null;
                if (packageDeclarator.getNode().getElementType() == edument.rakuidea.parsing.RakuTokenTypes.PACKAGE_DECLARATOR) {
