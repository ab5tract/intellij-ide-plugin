...
public class FormatterTest extends CommaFixtureTestCase {
    ...
    protected String getTestDataPath() {
        return "rakuidea-idea-plugin/testData/formatter";
    }
    ...
    private void reformatTest(String input, String output, BiConsumer<CommonCodeStyleSettings, RakuCodeStyleSettings> config) {
        myFixture.configureByText(RakuScriptFileType.INSTANCE, input);
        reformat(config);
        myFixture.checkResult(output);
    }
    ...
    private void reformatTest(String filename, BiConsumer<CommonCodeStyleSettings, RakuCodeStyleSettings> config) {
        myFixture.configureByFiles(filename + ".in.p6");
        reformat(config);
        myFixture.checkResultByFile(filename + ".out.p6", true);
    }

    private void reformat(BiConsumer<CommonCodeStyleSettings, RakuCodeStyleSettings> config) {
